...
def parse_telegram_init_data(init_data: str) -> TelegramInitData:
    ...
    try:
        ...
        logger.info("Parsed tg init data", extra={"parsed_data": parsed_data})
        # Извлекаем данные пользователя
        user_data = None
        if "user" in parsed_data:
            logger.info("User data in parsed data", extra={"user_data": parsed_data["user"]})
        ...
        logger.info("Create tg init data object", extra={"init_data_obj": init_data_obj})
...
def validate_telegram_init_data(init_data: str) -> bool:
    ...
    try:
        ...
        logger.info("Received hash", extra={"received_hash": received_hash})
        ...
        logger.info("Check string", extra={"check_string": check_string})
        # Создаем секретный ключ из токена бота
        secret_key = hmac.new(b"WebAppData", settings.TG_BOT_TOKEN.encode(), hashlib.sha256).digest()
        logger.info("Secret key", extra={"secret_key": secret_key})

        # Вычисляем хеш
        calculated_hash = hmac.new(secret_key, check_string.encode(), hashlib.sha256).hexdigest()
        logger.info("Calculated hash", extra={"calculated_hash": calculated_hash})
        # Сравниваем хеши
        is_eq = hmac.compare_digest(received_hash, calculated_hash)
        logger.info("Is equal", extra={"is_eq": is_eq})
        return is_eq
...
def is_telegram_data_fresh(auth_date: int, max_age_seconds: int = 86400) -> bool:
    """
...
def extract_user_data_from_init_data(init_data: str):
    """
    ...
    try:
        ...
        parsed_data = urllib.parse.parse_qs(init_data)
        if "user" in parsed_data:
            logger.info("Extracted user data from init data", extra={"user_data": parsed_data["user"][0]})
