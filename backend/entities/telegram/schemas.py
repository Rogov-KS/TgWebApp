...
class TelegramUserData(BaseModel):
    ...
    id: int = Field(..., description="Уникальный идентификатор пользователя в Telegram")
    ...
    language_code: str | None = Field(None, description="Код языка пользователя")
    is_premium: bool | None = Field(None, description="Является ли пользователь Premium")
...
class TelegramInitData(BaseModel):
    ...
    user: TelegramUserData | None = Field(None, description="Данные пользователя")
    chat_instance: str | None = Field(None, description="Идентификатор чата")
...
class TelegramAuthRequest(BaseModel):
    ...
    init_data: str = Field(..., description="Строка initData от Telegram Mini App")
